...
If `SEARXNG_ENGINES` is not set, MCP automatically uses `bing,duckduckgo,wikipedia`.
...
### LM Studio connects but tools do not respond or the model unloads

Use the config generated by `npm run print:mcp` again. It now defaults to a smaller LM Studio tool set:

```env
MCP_ENABLED_TOOLS=health_check,search_web,fetch_url,fetch_markdown,fetch_document,extract_structured_data,translate_text,close_browser
HEALTH_CHECK_BROWSER=0
REQUEST_TIMEOUT_MS=10000
SEARXNG_ENGINES=bing,duckduckgo,wikipedia
```

This avoids loading large browser-rendering tool schemas into LM Studio and prevents `health_check` from launching Chromium. To expose every MCP tool, set `MCP_ENABLED_TOOLS` to an empty string. To include Playwright in `health_check`, set `HEALTH_CHECK_BROWSER=1`.

Also verify Docker Desktop is running and SearXNG responds at:
